...
gs = gridspec.GridSpec(2,1, wspace=0.3, hspace=0.5)#, width_ratios=[1, 1])
...
cov = np.genfromtxt('../../RascalC/output/nd3_00/cov_matrix_RascalC_z0_nd3_00_r_60_160.dat')
fit = np.genfromtxt('../output/2pcf_auto_z0/nd3_00/rascalc_r_60_160/r_60_160_rascalc_model_mean.dat')
...
ax2.set_title("Auto-corr [nd = 0.001]")
#ax1.set_xticklabels([''])
...
rfit = fit[:,0][ii]
xifit = fit[:,1][ii]
ax2.plot(rfit,rfit**2 * xifit, label='fit model_mean', marker='.')
fit = np.genfromtxt('../output/2pcf_auto_z0/nd3_00/rascalc_r_25_150/r_25_150_rascalc_model_maxlike.dat')
ii=np.where((fit[:,0]<180) & (fit[:,0]>20)) 
rfit = fit[:,0][ii]
xifit = fit[:,1][ii]
ax2.plot(rfit,rfit**2 * xifit, label='fit maxlike_',  marker='.',color='g',linestyle='dotted')
